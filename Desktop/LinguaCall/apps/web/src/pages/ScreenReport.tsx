    ...
    <div className="space-y-6 text-sm">
      <div className="flex items-center gap-2">
        ...
        {report.status === 'failed' && (
          <span className="text-destructive text-xs">
            Report generation failed. Retry later.
          </span>
        )}
      </div>
      ...
      {fluency && (
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Avg WPM', value: fluency.avg_wpm },
            { label: 'Fillers', value: fluency.filler_count },
            { label: 'Pauses', value: fluency.pause_count }
          ].map(({ label, value }) => (
      ...
      <div className="text-xs text-muted-foreground space-y-1 pt-2 border-t border-border">
        ...
        {report.readyAt && <div>Ready at: {report.readyAt}</div>}
        {report.errorCode && <div>Error: {report.errorCode}</div>}
      </div>
    </div>
  );
}
