...
- default
- loading
- error banner 또는 inline message

### 금지

- social login button을 전제로 한 구조
...
- 예약 폼처럼 느껴지게 하되, 첫 화면에서는 행동 우선순위가 먼저 보여야 한다
- 현재 repo의 실제 기능 폭을 유지하면서도 launch path는 더 좁게 보이게 한다
...
- immediate 선택 시 `지금 시작`
- scheduled_once 선택 시 `이 시간으로 예약`
...
## 5. Screen 4, 홈 대시보드 / 세션 허브

### 목적

- 행동을 다시 시작하게 한다
- live 또는 upcoming session이 있으면 그것을 먼저 보이게 한다
- 최근 리포트 복습은 primary action을 가리지 않는 위치에 둔다

### 핵심 컴포넌트

- LaunchSpotlightCard
- AllowanceSummaryCard
- SessionComposerEntry
- LiveSessionCard
- ScheduledCallCard
- RecentReportList
- EmptyState
...
1. 지금 가장 먼저 확인해야 할 상태
2. 남은 체험 횟수 또는 분수
3. 새 연습 시작
4. upcoming scheduled_once 카드
5. 최근 리포트 3개
6. 과거 세션 상세 탐색
...
- primary action보다 더 눈에 띄는 보조 카드 남발
